...
Gainput is a C++ Open Source gainput library for games. It collects gainput data from different devices (like keyboards, mice or gamepads) and makes the inputs accessible through a unified interface. On top of that, inputs can be mapped to user-defined buttons and other advanced features are offered. Gainput aims to be a one-stop solution to acquire gainput from players for your game.
...
\tableofcontents
This page gives an overview on how to get Gainput into your game and use it for getting your players' gainput.
...
Gainput's most important class is gainput::InputManager. You should create one that you use throughout your game. Create some gainput devices using gainput::InputManager::CreateDevice(). And then, during your game loop, call gainput::InputManager::Update() every frame.

Some platform-specific function calls may be necessary, like gainput::InputManager::HandleMessage().

On top of your gainput::InputManager, you should create at least one gainput::InputMap and map some device-specific buttons to your custom user buttons using gainput::InputMap::MapBool() or gainput::InputMap::MapFloat(). This will allow you to more conventienly provide alternative gainput methods or enable the player to change button mappings.


\section sect_using Using Gainput
After everything has been set up, use gainput::InputMap::GetBool() or gainput::InputMap::GetFloat() (and related functions) anywhere in your game to get player gainput.
...
Shows how to use gainput gestures. Also shows how to implement your own custom gainput device. Uses the \ref sample_fw.
...
Shows how to record, play and serialize/deserialize gainput sequences. Uses the \ref sample_fw.
...
Evdev is used for the raw gainput variants. Evdev has permission issues on some Linux distributions where the devices (\c /dev/gainput/event*) are only readable by root or a specific group. If a raw device's state is gainput::InputDevice::DS_UNAVAILABLE this may very well be the cause.
...
Android NDK: All gainput is acquired through the NDK. Native App Glue is used for most inputs.
...
\section faq0 Why another library when gainput is included in most engines/libraries?
There are lots of other ways to acquire gainput, most are part of more complete engines or more comprehensive libraries. For one, Gainput is meant for those who are using something without gainput capabilities (for example, pure rendering engines) or those who are developing something themselves and want to skip gainput handling.

In the long run, Gainput aims to be better and offer more advanced features than built-in gainput solutions. That's the reason why more advanced features, like gainput recording/playback, remote syncing, gestures and external tool support, are already part of the library.
