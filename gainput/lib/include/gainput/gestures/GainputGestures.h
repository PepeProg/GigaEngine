...
/// Base class for all gainput gestures.
/**
 * Input gestures are a way to process basic gainput data into more complex gainput data. For example,
 ...
 *
 * Input gestures are basically just gainput devices that don't get their data from some hardware device
 * but from other gainput devices instead. Therefore gestures must also be created by calling
 ...
 * they should be good to go and their buttons can be used like any other gainput device button, i.e.
