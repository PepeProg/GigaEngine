...
/// Type of an gainput device button.
enum ButtonType
{
...
{
public:
	/// Type of an gainput device.
	enum DeviceType
	{
		DT_MOUSE,		///< A mouse/cursor gainput device featuring one pointer.
		DT_KEYBOARD,		///< A keyboard gainput device.
		DT_PAD,			///< A joypad/gamepad gainput device.
		DT_TOUCH,		///< A touch-sensitive gainput device supporting multiple simultaneous pointers.
		DT_BUILTIN,		///< Any controls directly built into the device that also contains the screen.
		DT_REMOTE,		///< A generic networked gainput device.
		DT_GESTURE,		///< A gesture gainput device, building on top of other gainput devices.
		DT_CUSTOM,		///< A custom, user-created gainput device.
		DT_COUNT		///< The count of gainput device types.
	};

	/// Variant of an gainput device type.
	enum DeviceVariant
	{
		DV_STANDARD,		///< The standard implementation of the given device type.
		DV_RAW,			///< The raw gainput implementation of the given device type.
		DV_NULL			///< The null/empty implementation of the given device type.
	};

	/// State of an gainput device.
	enum DeviceState
	{
		DS_OK,			///< Everything is okay.
		DS_LOW_BATTERY,		///< The gainput device is low on battery.
		DS_UNAVAILABLE		///< The gainput device is currently not available.
	};

	static const unsigned AutoIndex = unsigned(-1);

	/// Initializes the gainput device.
	/**
	 * Do not instantiate any gainput device directly. Call InputManager::CreateDevice() instead.
	 */
	...
	/// or a recorded gainput sequence, false otherwise.
