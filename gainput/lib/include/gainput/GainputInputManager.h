 ...
 *
 * Note that destruction of an InputManager invalidates all gainput maps based on it and all devices created
 * through it.
 */
...
public:
	...
	/// [ANDROID ONLY] Lets the InputManager handle the given gainput event.
    ...
    void HandleDeviceInput(DeviceInput const& gainput);
#endif

	/// Updates the gainput state, call this every frame.
	/**
	...
	/// Updates the gainput state and the manager's time, call this every frame.
	/**
	...
	/// Creates an gainput device and registers it with the manager.
	/**
	 * \tparam T The gainput device class, muste be derived from InputDevice.
	 ...
	 * \return The ID of the newly created gainput device.
	 */
	...
	/// Creates an gainput device, registers it with the manager and returns it.
	/**
	 * \tparam T The gainput device class, muste be derived from InputDevice.
	 * \param variant Requests the specified device variant. Note that this is only
	 * a request. Another implementation variant of the device may silently be instantiated
	 * instead. To determine what variant was instantiated, call InputDevice::GetVariant().
	 * \return The newly created gainput device.
	 */
	...
	/// Returns the gainput device with the given ID.
	/**
	 * \return The gainput device or 0 if it doesn't exist.
	 */
	...
	/**
	 * If there are listeners registered, all gainput devices will have to record their state changes. This incurs extra runtime costs.
	 */
	...
	/// Enable/disable debug rendering of gainput devices.
...
/// Interface for modifiers that change device gainput states after they have been updated.
