...
/**
 * This is the interface that should be used to get gainput. You can have several maps that are used
 * simultaneously or use different ones depending on game state. The user button IDs have to be unique per gainput map.
 *
...
public:
	/// Initializes the map.
	/**
	 * \param manager The gainput manager used to get device inputs.
	 * \param name The name for the gainput map (optional). If a name is provided, it is copied to an internal buffer.
	...
	/// Returns the gainput manager this gainput map uses.
	 ...
	 * as 0.0f. The absolute gainput value is used in order to determine if the gainput value falls within the dead
